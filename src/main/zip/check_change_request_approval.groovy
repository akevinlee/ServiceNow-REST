...
for (def changeId : changeIds.sort()) {
    if (helper.changeRequestExists(changeId)) {
        def actualState = helper.changeRequestApprovalState(changeId);
        if (actualState == approvalName) {
            println "Found \"${changeId}\" with the correct approval state: ${approvalName}.";
            stateCount++;
        } else {
            if (failMode == FailMode.FAIL_FAST) {
                helper.exitFailure("Change Request \"${changeId}\" has an different approval state: ${actualState}.");
            } else {
                println "Change Request \"${changeId}\" has an different approval state: ${actualState}.";
            }
        }
    } else {
        if (failMode == FailMode.FAIL_FAST) {
...
if (failMode == FailMode.FAIL_ON_ANY_FAILURE) {
    if (!changeIds || (stateCount != totalNum)) {
        helper.exitFailure("Only found correct approval state on ${stateCount} out of ${totalNum} Change Requests.");
    }
}
println "Found the correct approval state on ${stateCount} out of ${totalNum} Change Requests.";
