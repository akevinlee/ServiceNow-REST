...
for (def changeId : changeIds.sort()) {
    ...
    if (sysId != null) {
        ...
        println "Set state of Change Request \"${changeId}\" to: ${state}.";
        stateCount++;
    } else {
...
if (failMode == FailMode.FAIL_ON_ANY_FAILURE) {
    if (!changeIds || (stateCount != totalNum)) {
        helper.exitFailure("Only set state on ${stateCount} out of ${totalNum} Change Requests.");
    }
}
println "Set the state on ${stateCount} out of ${totalNum} Change Requests.";
